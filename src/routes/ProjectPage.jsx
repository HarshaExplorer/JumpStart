...
const ProjectPage = ({ token }) => {
  ...
  useEffect(() => {
    ...
    const fetchComments = async () => {
      try {
        const projectId = parseInt(pid); // Convert pid to number
        const { data: commentsData, error } = await database
          .from("comments")
          .select()
        ...
        if (commentsData && commentsData.length > 0) {
          ...
          console.log("No comments found for project ID:", pid);
        }
      } catch (error) {
        console.error("Error fetching comments:", error.message);
      }
  ...
  return (
    <>
      {project && (
        <Stack gap={4}>
          <h1 className="abril-fatface-regular mt-3 mx-auto">
            {project.title}
          </h1>
          <hr />
              ...
              {Number(project_life) > 0 && (
                <InputGroup className="m-2 mx-auto" style={{ width: "80%" }}>
                  <Button
                    ...
                    disabled={token || Number(project_life) <= 0 ? false : true}
                  >
                    Back this project
                  </Button>
                  <Form.Control
